	...
	for (let memberIdx = 0; memberIdx < profiles.length; memberIdx++) {
		...
		switch (skillName) {
			...
			case "Dream Shard Magnet S (Aura Sphere)": {
				...
				if (skillName === "Dream Shard Magnet S (Aura Sphere)") {
					profile.skillValue2 = Math.ceil(
						getSkillSubValue(skillName, skillLevel) *
			...
			case "Energizing Cheer S (Heal Pulse)":
				profile.skillValue = getSkillValue(skillName, skillLevel);
				profile.skillValue2 = getSkillValue(skillName, skillLevel);
				if (
					profiles.some(
						(x) => x.iv.pokemon.skill === "Berry Burst (Draco Meteor)",
					)
				) {
					const bonus = skillLevel <= 2 ? 1 : skillLevel <= 5 ? 2 : 3;
					profile.skillValue2 += bonus;
				}
				break;
