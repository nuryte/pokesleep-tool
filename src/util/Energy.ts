...
import type { ExpertEffects } from "./StrengthParameter";
...
class Energy {
	...
	constructor(iv: PokemonIv) {
		this._iv = iv;
		this._wakeMax = iv.activeSubSkills.some(
			(x) => x.name === "Energy Recovery Bonus",
		)
			? 105
			: 100;
	}
	...
	calculate(
		...
		if (param.isEnergyAlwaysFull) {
			...
			this.calculateEnergyForEvents(
				events,
				myRestoreEnergy,
				sleepRecovery,
				sleepRecovery,
				bonus.energyFromDish,
			);
		...
		const total = this.calculateAverageEfficiency(
			efficiencies.filter((x) => x.isInPeriod),
		);
		const awake = this.calculateAverageEfficiency(
			efficiencies.filter((x) => x.isInPeriod && x.isAwake),
		);
		const asleep = this.calculateAverageEfficiency(
			efficiencies.filter((x) => x.isInPeriod && !x.isAwake),
		);

		// calculate Sneaky Snacking
		const showSkillStock =
			param.tapFrequencyAwake !== NoTap && param.tapFrequencyAsleep === NoTap;
	 ...
	 */
	calculateEnergyForEvents(
		events: EnergyEvent[],
		myRestoreEnergy: number,
		...
		for (let i = 1; i < events.length; i++) {
			...
			curEvent.energyBefore = Math.max(
				0,
				prevEvent.energyAfter - energyConsumed,
			);
			...
			if (curEvent.type === "cook") {
				...
				curEvent.energyAfter = Math.min(
					150,
					curEvent.energyAfter + myRestoreEnergy,
				);
			} else if (curEvent.type === "wake") {
				curEvent.energyAfter = Math.min(
					this._wakeMax,
					curEvent.energyAfter + sleepRecovery,
				);
	 ...
	 */
	splitEventAndEfficiencyByPeriod(
		events: EnergyEvent[],
		efficiencies: EfficiencyEvent[],
		param: EnergyParameter,
	): void {
		...
		for (let i = 0; i < events.length; i++) {
			...
			const energy = param.isEnergyAlwaysFull
				? event.energyAfter
