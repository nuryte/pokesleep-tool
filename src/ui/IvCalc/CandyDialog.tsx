...
const CandyDialog = React.memo(
	({
		...
		const shouldReset = React.useCallback(() => {
			...
			if (
				iv.level === levelInfo.currentLevel &&
		...
		const onLevelInfoChange = React.useCallback(
			(value: LevelInfo) => {
				// Check if currentLevel changed
				if (value.currentLevel !== levelInfo.currentLevel) {
					setMaxExpLeft(
						calcExp(value.currentLevel, value.currentLevel + 1, iv),
					);
...
const LevelForm = React.memo(
	({
		...
		const handleCurrentLevelChange = React.useCallback(
			(level: number) => {
				onLevelInfoChange({
					...levelInfo,
					currentLevel: level,
		...
		const handleTargetLevelChange = React.useCallback(
			(level: number) => {
				onLevelInfoChange({
					...levelInfo,
					targetLevel: level,
		...
		const handleExpGotChange = React.useCallback(
			(value: number) => {
				onLevelInfoChange({
					...levelInfo,
					expGot: value,
		...
		const handleExpLeftChange = React.useCallback(
			(value: number) => {
				onLevelInfoChange({
					...levelInfo,
		...
		return (
			<>
				<StyledIcon>
					<PokemonIcon
						idForm={levelInfo.iv.idForm}
						shiny={levelInfo.iv.shiny}
						size={80}
					/>
				</StyledIcon>
				<StyledLevel>
					<div className="level">
						...
						<div className="expLeft">
							<StyledSlider
								value={levelInfo.expGot}
								min={0}
								max={maxExpLeft - 1}
								onChange2={handleExpGotChange}
							/>
							<NumericSliderInput
								value={maxExpLeft - levelInfo.expGot}
								size="small"
								startAdornment={
				...
				<div className="form">
					...
					<section>
						...
						<ToggleButtonGroup
							size="small"
							exclusive
							value={config.candyBoost}
							onChange={onCanyBoostChange}
						>
...
const calculateDetailCandy = (
	...
	const normalCandyResult: CalcLevelResult = calcLevelByCandy(
		iv,
				...
				<div className="form">
					...
					<CollapseEx show={config.candyBoost !== "none"}>
						...
						<section>
							...
							{config.boostPolicy === "candy" && (
								<div>
									<CandyIcon />
									<NumericSliderInput
										sx={{ width: "3rem", fontSize: "0.9rem" }}
										min={0}
										max={config.pokemonCandy}
...
const CandyResultPreview = React.memo(
	({ iv, value }: { iv: PokemonIv; value: CalcLevelBoostResult }) => {
		const { t } = useTranslation();
		...
		if (!isMaxLevel) {
			const expToNextLevel =
				calcExp(value.level, value.level + 1, iv) - value.expGot;
			expToGo = [
				t("exp to go1"),
				formatWithComma(expToNextLevel),
				t("exp to go2"),
			]
				.filter((x) => x !== "")
				.join(" ");
		...
		return (
			<StyledPreview>
				...
				<span className="shard">
					...
					{value.extraShards > 0 && (
						<footer>(+{formatWithComma(value.extraShards)})</footer>
					)}
				</span>
			</StyledPreview>
		);
...
const SleepResultPreview = React.memo(
	({
		iv,
		level,
		value,
